    ...
    public class SettingsPageViewModel : PageViewModelBase
    {
        ...
        private async Task SelectMcpServerPortAsync()
        {
            var result = await _dialogService.ShowInputDialogAsync(
                ...
                $"{IMcpServerService.PortDefault}", // McpServerPortDefault 5000
                Translate("Ok"),
                Translate("Cancel"),
                KeyboardType.Numeric,
                (portText) => !string.IsNullOrEmpty(portText) && int.TryParse(portText, out int portNumber) && portNumber >= IMcpServerService.PortMin && portNumber <= IMcpServerService.PortMax,
                DisappearingToken);
            ...
            {
                if (intValue < IMcpServerService.PortMin || intValue > IMcpServerService.PortMax)
                {
